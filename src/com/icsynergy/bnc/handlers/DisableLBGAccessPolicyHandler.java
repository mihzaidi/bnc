...
public class DisableLBGAccessPolicyHandler implements PostProcessHandler {
	...
	Long userKey = null;
	AbstractGenericOrchestration abc = null;
	@Override
	public EventResult execute(long arg0, long arg1, Orchestration orchestration) {
		...
		Account account = (Account) modParams.get("Account");
		log.info("orchestration.getParameters() =  " + orchestration.getParameters() );
    	String entityId = orchestration.getTarget().getEntityId();
		String targetType = orchestration.getTarget().getType();
		String userKeyStr = account.getUserKey();
		log.info("entityId = " + entityId + "targetType = " + targetType + "userKeyStr = " + userKeyStr);
		disableADAccount(userKeyStr);
